...
OrkSpearShield::OrkSpearShield() : Opponent()
{
	totalHeal = 100;
...
int OrkSpearShield::getSpriteNumberInSpritesArray()
{
	return spriteNumberInSpritesArray;
}

void OrkSpearShield::setSpriteNumberInSpritesArray(int num)
{
	spriteNumberInSpritesArray = num;
}

int OrkSpearShield::action()
{
	doAnimation = NOTHING;
...
void OrkSpearShield::animationForOpponentMoving(int animation, sf::RenderWindow * tempWindow, Render * tempRender, Player * player)
{
	if (animation == FIGHT)
	{
		...
		if (opponentAnimationStep > 7)
		{
			opponentAnimationStep = 0;
			isLastFrame = true;
		}
		tempGraphics.setTextureRect(sf::IntRect(34 * opponentAnimationStep, 34 * (4 + lastOpponentDirection - 1), 34, 34));
