...
int Skeleton::getSpriteNumberInSpritesArray()
{
	return spriteNumberInSpritesArray;
}

void Skeleton::setSpriteNumberInSpritesArray(int num)
{
...
int Skeleton::action()
{
	...
	if (detectChampion == true)
	{
		...
		{
			// goni postac												--> to jest do dopracowania i powiekszenia
	...
	{
		//jesli jest w swoim kwadracie to nic jesli nie to wraca		--> to jest do dopracowania i powiekszenia
...
void Skeleton::animationForOpponentMoving(int animation, sf::RenderWindow * tempWindow, Render * tempRender, Player *player)
{
	if (animation == FIGHT)
	{
